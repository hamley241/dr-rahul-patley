// src/breathing.jsx — mindful breathing tool
// Self-contained: add/remove by toggling <Breathing/> in app.jsx
// Modes are declarative — add a new pattern to BREATHING_MODES to extend.
...
// Smooth easing so the ring swells and settles naturally instead of hitting a linear wall at phase boundaries.
const easeInOut = (t) => t < 0.5 ? 2 * t * t : 1 - Math.pow(-2 * t + 2, 2) / 2;

function Breathing() {
  if (window.__TWEAKS && window.__TWEAKS.showBreathing === false) return null;
  const [modeKey, setModeKey] = React.useState('box');
  const [running, setRunning] = React.useState(false);
  // Countdown / cycle number are the only things that need React state — they change once per second,
  // not per frame. The ring scale is driven via refs + direct transform mutation to avoid 60fps re-renders.
  ...
  // Keep live refs of mode/phaseIdx so the rAF loop sees fresh values without restarting on every change.
  const modeRef = React.useRef(mode);
  const phaseIdxRef = React.useRef(phaseIdx);
  React.useEffect(() => { modeRef.current = mode; }, [mode]);
  React.useEffect(() => { phaseIdxRef.current = phaseIdx; }, [phaseIdx]);

  const applyScale = React.useCallback((s) => {
    if (ringRef.current) ringRef.current.style.transform = `scale(${s})`;
    if (haloRef.current) haloRef.current.style.transform = `scale(${s})`;
  }, []);

  // Single rAF loop — runs while `running`, reads live phase from refs, writes transforms directly.
  React.useEffect(() => {
    if (!running) {
...
Object.assign(window, { Breathing, BREATHING_MODES, BREATHING_REFERENCES, BREATHING_TECHNIQUES, BreathingEvidenceModal });
