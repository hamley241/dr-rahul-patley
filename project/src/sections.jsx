...
function Booking() {
  ...
  const submit = (e) => {
    ...
    if (Object.keys(missing).length) {
      ...
      // Scroll to first missing area
      setTimeout(() => {
        const target = missing.date || missing.slot ? document.querySelector('.booking-left') : document.querySelector(`[data-error-anchor="${Object.keys(missing).find((k) => missing[k])}"]`);
        if (target) {
